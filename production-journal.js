...
(function(root,factory){
  ...
  function create(options){
    ...
    async function reconcilePending(remote,target,expectedToken,resolver,recoveryContext){
      ...
      const result=await journal.reconcilePending(remote,target,expectedToken,resolver,recoveryContext);
    ...
    return {initialize,acceptRemote,stage,flush,useRemote,reconcilePending,resumeSavedDraft,status,metadata:journal.metadata,snapshot:journal.snapshot,ready:journal.ready,exportState,current:()=>clone(viewBaseline),rawCurrent:journal.current};
  }
